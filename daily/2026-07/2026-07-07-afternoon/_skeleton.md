...
advisory 骨架文件. 13:00 cron 抵达时由 Una cron prompt 读取, 进入阶段 3 落盘.
时戳: 2026-07-07 10:42 CST. Una 主审在场. 不变量 #6 (loop+darwin) 在 SKILL.md v0.7.9 落定.

---

## 选材: Anthropic 这家公司, 36 小时内五件事砸下来

### 不变量自检 (2026-07-07 Gen 3 启用)

- Loop: 选完后 SKILL.md 长出什么? → "美方头部 AI 公司 IPO 前夜, 客户/市场/内部/政治四方同时施压, 是 2026 H2 资本战新结构" (跨日可复用的判断框架, 新结构).
- Darwin: 明天 cron 能继承什么? → "Anthropic 作为美方头部 AI 角色" 这条 baseline 持续生效, 直到 IPO 真正发生或别家取代. 可遗传.
- 双 YES. 锁定本选材.

### 选材理由 (cron 抵达时直接用)

今天 7/6 (美东) 这一天, 美国头部 AI 公司 Anthropic 同时被四件事拉:
...
为什么这件: 当一家头部 AI 公司被这四股力量 (资本 + 市场 + 内部 + 政治) 同日拉扯, 它当下做的所有动作 (IPO 估值、Fable 5 造势、芯片定制、Karpathy 入伙) 都互相牵连, 是同一场博弈的不同手. 早上我说"国家大基金进场", 中国 AI 在变钱的方向; 现在这家美方头部在变另一件事. 早是国家级资本进场 (单点深), 午后是市场化资本 + 政治 + 技术叙事 (多元角力).
...
- 资本: Anthropic 估值 $965B (5 月), OpenAI $852B (3 月), The Daily Upside 报道两家下半年都 IPO, Anthropic 最早 10 月. 守门标记 [Una 现场补料-待 cron 验]: $965B/$852B + Karp 引述不在 morning raw.md, 是 Una web_extract The Daily Upside URL 现场抓的 (URL 已验证可达). 13:00 cron 阶段 1/2 必须 (a) sub-agent 重采 raw 把 Karp + 估值补进 raw/2026-07-07-afternoon.md; (b) Una 主审二次 grep 才能进 stat-card. 不满足则降级: Karp 段改"路透 7/6 报道 token 模式被质疑"匿名, 删 $965B/$852B.
- 市场: Palantir CEO Karp 说 token 付费模式"something has gone completely wrong". Uber / Microsoft / Salesforce / Meta 已经在限制员工 AI 支出. 守门标记同上, Karp + Palantir 都是现场补料. cron 守门前不进 publish.
...
3. 阶段 3: 跑 Humanizer audit pass (per SKILL.md v0.7.9 + references/humanizer-cron-integration.md 33 patterns 自审 + 改写). 然后 v4-Daily.html 模板 render, TTS MiniMax CN danya_xuejie 60 秒生成 voice.mp3, git commit + push origin main, wrangler deploy
4. 阶段 4: TG 7069165189 回报 "Una 选: Anthropic 同一天被四股力拉扯. 13:00. <URL>"

## 不变量 #6 自检回填 (cron 跑完时)

- Loop: 本日 cron 结束后, 是否在 SKILL.md 长新东西? → 若有, patch v0.7.10; 若无, 写进 STATE.md 但不升 skill.
- Darwin: 明天 21:00 cron 是否继承 "Anthropic 作为美方头部 AI 角色" 这条 baseline? → 是 → 不动; 否 → 重审.

---

主审签: Una <una@leftink.local>
时戳: 2026-07-07 10:42 CST (写于 cron 抵达前 2h 18min)
SKILL.md 版本: v0.7.9 (Humanizer audit pass 已加, Pitfall 37)